...
<div class="matrix-layout">
  ...
  <div class="control-deck">
    <div class="command-btn-group">
      ...
      <!-- Tombol Kelola / Hapus Data TiDB Cloud -->
      <button type="button" class="btn-action btn-manage-data" id="btnOpenDbManager" onclick="openDbManagerModal()" title="Buka Database Manager untuk melihat, memuat, dan menghapus data sesi di TiDB Cloud">
        ...
        <div class="raw-table-wrapper" id="matrixTableView" style="display:none;padding:12px;">
          <div class="table-view-header">
            <div class="table-info-badge">
              ...
              <strong id="tableActiveObjTitle">Gentong</strong>
              <span class="badge-count" id="tableActiveObjCount">10 Baris</span>
            </div>
            <div class="table-actions-group">
              <button class="btn-micro-danger" onclick="deleteCurrentActiveObject()" title="Hapus seluruh data untuk objek yang sedang dibuka">
                ...
                Hapus Data Objek Ini
              </button>
              <button class="btn-micro-primary" onclick="openDbManagerModal()">
...
<!-- ========================================================================
     MODAL: DATABASE MANAGER & DATA VIEWER (TiDB Cloud)
     ======================================================================== -->
<div class="scan-config-modal-backdrop" id="modalDatabaseManagerBackdrop" style="display:none;" onclick="closeDbManagerModal(event)">
  <div class="scan-config-modal-card db-manager-card" onclick="event.stopPropagation()">
    <div class="scan-modal-head">
      <div style="display:flex;align-items:center;gap:12px;">
        <div class="scan-modal-icon" style="background:rgba(56,189,248,0.15);color:#38BDF8;border-color:rgba(56,189,248,0.35);">
        ...
        <div>
          <h3>DATABASE &amp; SESSION MANAGER</h3>
          <p class="scan-modal-sub">Lihat daftar objek, periksa detail sesi rekaman scan, dan kelola/hapus data di TiDB Cloud.</p>
        </div>
      </div>
      <button class="scan-modal-close" onclick="closeDbManagerModal()" title="Tutup">&times;</button>
    </div>
    ...
    <div class="db-manager-body">
      ...
      <div class="db-summary-strip">
        <div class="db-metric-box">
          ...
          <strong id="dbMgrTotalObjects" style="color:#A78BFA;">--</strong>
        </div>
        <div class="db-metric-box">
          <span>TOTAL SESI SCAN</span>
          <strong id="dbMgrTotalSessions" style="color:#60A5FA;">--</strong>
        </div>
        <div class="db-metric-box">
          <span>TOTAL REKAMAN ROW</span>
          <strong id="dbMgrTotalRecords" style="color:#FBBF24;">--</strong>
        </div>
        <div class="db-metric-box">
          <span>DATABASE TARGET</span>
          <strong style="color:var(--accent-emerald);">TiDB Cloud (Live)</strong>
        </div>
      </div>

      <!-- Filter & Action Controls -->
      <div class="db-controls-bar">
        <div class="db-filter-item">
          <label for="dbMgrSelectObject">Filter Objek:</label>
          <select id="dbMgrSelectObject" class="db-filter-select" onchange="filterDbManagerByObject(this.value)">
            <option value="ALL">📦 Semua Objek</option>
          </select>
        </div>
        <button class="btn-micro-secondary" id="btnRefreshDbManager" onclick="fetchDbManagerData(true)" title="Muat ulang data dari TiDB Cloud">
          <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="23 4 23 10 17 10"></polyline><polyline points="1 20 1 14 7 14"></polyline><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path></svg>
          Sinkron TiDB
        </button>
      </div>

      <!-- Table of Sessions / Scans -->
      <div class="db-table-scroll-container">
        <table class="db-manager-table">
          <thead>
            <tr>
              <th>Nama Objek</th>
              <th>ID Sesi Scan</th>
          ...
          <tbody id="dbMgrTableBody">
            <tr>
              <td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted);">Memuat data sesi dari TiDB Cloud...</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="db-danger-zone">
        <div class="danger-zone-info">
          <strong>Pembersihan Data Server:</strong>
          <span>Data yang dihapus akan langsung terhapus dari server TiDB Cloud dan penyimpanan lokal.</span>
        </div>
        <div class="danger-zone-btns">
          <button type="button" class="btn-danger-action" id="btnDeleteFilteredObj" onclick="deleteFilteredObjectGroup()">
            ...
            <span id="btnDeleteFilteredObjText">Hapus Data Objek Ini</span>
          </button>
          <button type="button" class="btn-danger-action outline" onclick="deleteAllDatabaseRecords()">
            ⚠️ Kosongkan Semua Scan
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="scan-modal-footer">
      <button class="btn-modal-cancel" onclick="closeDbManagerModal()">Tutup</button>
    </div>
  </div>
</div>
...
function updateRawTable() {
  ...
  // Update Data Table header info
  const titleEl = document.getElementById('tableActiveObjTitle');
  const countEl = document.getElementById('tableActiveObjCount');
  if (titleEl) titleEl.textContent = APP_STATE.objectName || 'Gentong';
  if (countEl) countEl.textContent = `${APP_STATE.matrixData.length} Baris Level`;
}

// ==========================================
// 18. DATABASE & SESSION MANAGER HANDLERS (TiDB Cloud)
// ==========================================
let DB_MGR_DATA = {
  objects: [],
  sessions: [],
...
function openDbManagerModal() {
  const modal = document.getElementById('modalDatabaseManagerBackdrop');
  if (modal) modal.style.display = 'flex';
  fetchDbManagerData(false);
}

function closeDbManagerModal(event) {
  if (event && event.target !== document.getElementById('modalDatabaseManagerBackdrop')) return;
  const modal = document.getElementById('modalDatabaseManagerBackdrop');
  if (modal) modal.style.display = 'none';
}

function fetchDbManagerData(isManual = false) {
  const refreshBtn = document.getElementById('btnRefreshDbManager');
  if (refreshBtn) refreshBtn.classList.add('spinning');

  fetch('/matrix-data/history?limit=100')
    .then(res => res.json())
    .then(data => {
      if (refreshBtn) refreshBtn.classList.remove('spinning');
      if (data && (data.status === 'success' || data.status === 'empty')) {
        DB_MGR_DATA.objects = data.available_objects || [];
        DB_MGR_DATA.sessions = data.available_sessions || [];
        DB_MGR_DATA.records = data.records || [];

        // Update stats
        const elObj = document.getElementById('dbMgrTotalObjects');
        const elSess = document.getElementById('dbMgrTotalSessions');
        const elRec = document.getElementById('dbMgrTotalRecords');

        let totalRecCount = 0;
        DB_MGR_DATA.objects.forEach(o => totalRecCount += (parseInt(o.total_records) || 0));

        if (elObj) elObj.textContent = `${DB_MGR_DATA.objects.length} Objek`;
        if (elSess) elSess.textContent = `${DB_MGR_DATA.sessions.length} Sesi`;
        if (elRec) elRec.textContent = `${totalRecCount || (data.records ? data.records.length : 0)} Baris`;

        // Populate modal object filter dropdown
        populateDbManagerObjectSelect();

        // Render table
        renderDbManagerTable(DB_MGR_DATA.filteredObject);

        if (isManual) {
          showToast('success', 'Database Dimuat', `Berhasil memuat <b>${DB_MGR_DATA.objects.length} objek</b> dan <b>${DB_MGR_DATA.sessions.length} sesi</b> dari TiDB Cloud.`);
        }
      }
    })
    .catch(err => {
      if (refreshBtn) refreshBtn.classList.remove('spinning');
      console.warn("[DB Manager Error]", err);
      if (isManual) {
        showToast('warning', 'Gagal Memuat', 'Tidak dapat mengambil data dari database TiDB Cloud.');
      }
    });
}

function populateDbManagerObjectSelect() {
  const sel = document.getElementById('dbMgrSelectObject');
  if (!sel) return;
  const current = DB_MGR_DATA.filteredObject || 'ALL';

  sel.innerHTML = '<option value="ALL">📦 Semua Objek</option>';
  DB_MGR_DATA.objects.forEach(obj => {
    const opt = document.createElement('option');
    opt.value = obj.object_name;
    opt.textContent = `${obj.object_name} (${obj.total_records} data)`;
    if (obj.object_name === current) opt.selected = true;
    sel.appendChild(opt);
  });
}

function filterDbManagerByObject(selectedName) {
  DB_MGR_DATA.filteredObject = selectedName;
  const btnText = document.getElementById('btnDeleteFilteredObjText');
  if (btnText) {
    btnText.textContent = selectedName === 'ALL' ? 'Hapus Data Objek Ini' : `Hapus Seluruh Data '${selectedName}'`;
  }
  renderDbManagerTable(selectedName);
}

function renderDbManagerTable(filterObj = 'ALL') {
  const tbody = document.getElementById('dbMgrTableBody');
  if (!tbody) return;

  let items = [];

  if (DB_MGR_DATA.sessions && DB_MGR_DATA.sessions.length > 0) {
    items = DB_MGR_DATA.sessions;
  } else if (DB_MGR_DATA.objects && DB_MGR_DATA.objects.length > 0) {
    items = DB_MGR_DATA.objects.map(o => ({
      session_id: 'ALL_ROWS',
  ...
  if (filterObj && filterObj !== 'ALL') {
    items = items.filter(item => item.object_name === filterObj);
  }
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted);">
          Tidak ada data sesi scan yang ditemukan untuk filter ini di TiDB Cloud.
        </td>
      </tr>
    `;
    return;
  ...
  items.forEach(sess => {
    ...
    const peakBadge = sess.peak_cps >= 100 
      ? `<span class="badge-cps-hotspot">${sess.peak_cps} CPS</span>` 
      : `<span class="badge-cps-normal">${sess.peak_cps || 0} CPS</span>`;

    tr.innerHTML = `
      <td><strong style="color:#C4B5FD;">${sess.object_name}</strong></td>
      <td><code class="code-session-id">${sess.session_id || 'ID Auto'}</code></td>
      <td><span class="badge-level-count">${sess.total_records || sess.max_height || 1} Level</span></td>
      <td>${peakBadge}</td>
      <td style="font-family:var(--font-mono);font-size:11px;color:var(--text-secondary);">${sess.last_ts || sess.last_created || '-'}</td>
      <td style="text-align:center;">
        <div class="db-row-actions">
          <button class="btn-db-action load" onclick="loadSpecificSession('${sess.session_id}', '${sess.object_name}')" title="Tampilkan ke visualisasi Matrix 2D/3D">
            👁️ Tampilkan
          </button>
          <button class="btn-db-action delete" onclick="deleteSpecificSession('${sess.session_id}', '${sess.object_name}')" title="Hapus sesi ini dari TiDB Cloud">
            🗑️ Hapus
          </button>
        </div>
      </td>
...
function loadSpecificSession(sessionId, objectName) {
  closeDbManagerModal();
  const selectObj = document.getElementById('selectActiveObject');
  if (selectObj) selectObj.value = objectName;
  onObjectSelectChange(objectName);
  showToast('info', 'Memuat Sesi', `Menampilkan visualisasi data pemindaian objek <b>${objectName}</b>.`);
}

function deleteSpecificSession(sessionId, objectName) {
  const confirmMsg = sessionId && sessionId !== 'ALL_ROWS'
    ? `Apakah Anda yakin ingin MENGHAPUS data sesi '${sessionId}' untuk objek '${objectName}' dari TiDB Cloud?`
    : `Apakah Anda yakin ingin MENGHAPUS seluruh data pemindaian objek '${objectName}' dari TiDB Cloud?`;

  if (!confirm(confirmMsg)) return;

  const payload = (sessionId && sessionId !== 'ALL_ROWS')
    ? { type: 'session', session_id: sessionId, object_name: objectName }
    : { type: 'object', object_name: objectName };

  performDeleteRequest(payload, `Sesi '${sessionId || objectName}'`);
}

function deleteSpecificObject(objectName) {
  if (!objectName || objectName === 'ALL') {
    showToast('warning', 'Pilih Objek', 'Silakan pilih objek spesifik terlebih dahulu untuk dihapus.');
    return;
  }
  if (!confirm(`⚠️ PERINGATAN: Seluruh rekaman scan untuk objek '${objectName}' akan DIHAPUS PERMANEN dari TiDB Cloud.\n\nApakah Anda yakin ingin melanjutkan?`)) {
    return;
  }
  performDeleteRequest({ type: 'object', object_name: objectName }, `Objek '${objectName}'`);
}

function deleteFilteredObjectGroup() {
  const current = DB_MGR_DATA.filteredObject;
  if (current === 'ALL') {
    const selectObj = document.getElementById('selectActiveObject');
    const activeObj = selectObj ? selectObj.value : 'ALL';
    if (activeObj && activeObj !== 'ALL') {
      deleteSpecificObject(activeObj);
    } else {
      showToast('warning', 'Pilih Objek', 'Silakan pilih objek tertentu di dropdown filter sebelum menghapus.');
    }
  } else {
    deleteSpecificObject(current);
  }
}

function deleteCurrentActiveObject() {
  const activeObj = APP_STATE.objectName;
  if (!activeObj || activeObj === 'ALL') {
    showToast('warning', 'Objek Tidak Valid', 'Belum ada objek aktif yang dipilih untuk dihapus.');
    return;
  }
  deleteSpecificObject(activeObj);
}

function deleteAllDatabaseRecords() {
  const code = prompt("⚠️ KONFIRMASI RESET TOTAL:\nKetik 'RESET' untuk menghapus SELURUH data pemindaian di TiDB Cloud:");
  if (code !== 'RESET') {
    if (code !== null) alert("Konfirmasi dibatalkan. Kata kunci tidak cocok.");
    return;
  }

  performDeleteRequest({ type: 'all' }, "Seluruh Database Scan");
}

function performDeleteRequest(payload, itemLabel) {
  fetch('/matrix-data/delete', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-CSRF-TOKEN': '{{ csrf_token() }}'
  ...
  .then(data => {
    if (data.status === 'success') {
      showToast('success', 'Data Dihapus', `${itemLabel} berhasil dihapus dari <b>${data.db_target === 'tidb_cloud' ? 'TiDB Cloud' : 'Database'}</b> (${data.deleted_count} baris).`);
      logTerminal(`<span class="log-badge-alert">[DB DELETE]</span> ${itemLabel} berhasil dihapus (${data.deleted_count} data).`);
      
      // Refresh both UI list and DB Manager
      fetchDbManagerData(false);
      refreshAvailableObjects(false);
      loadObjectDataFromTiDB('ALL', false);
    } else {
      showToast('warning', 'Gagal Menghapus', data.message || 'Terjadi kesalahan saat menghapus data.');
    }
  })
  .catch(err => {
    console.error("[Delete Request Error]", err);
    showToast('warning', 'Kesalahan Jaringan', 'Gagal mengirim permintaan hapus ke server.');
  });
}
